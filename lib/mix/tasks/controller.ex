defmodule Mix.Tasks.Procon.Controller do
  ...
  def run(args) do
    schema =
      OptionParser.parse(args, strict: [schema: :string]) |> elem(0) |> Keyword.get(:schema)
    ...
    create_file(
      Path.join(["lib", app_name, "message_controllers", schema |> Macro.underscore()]) <>
        "_controller.ex",
      controller_template(opts)
    )
